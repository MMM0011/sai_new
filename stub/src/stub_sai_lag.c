    ...
    _In_ sai_attribute_t *attr_list)
{
    ...
    lag_db.members[lag_member_db_id].lag_oid = *(sai_object_id_t*)lag_id;

    const sai_attribute_value_t *port_id;
    uint32_t port_id_idx;
    
    status = find_attrib_in_list(attr_count, attr_list, SAI_LAG_MEMBER_ATTR_LAG_ID, &port_id, &port_id_idx);
    if (status != SAI_STATUS_SUCCESS) {
        printf("LAG_ID attribute not found.\n");
        return status;
    }                 
    lag_db.members[lag_member_db_id].port_oid = *(sai_object_id_t*)port_id;
    ...
    uint32_t lag_db_id;
    stub_object_to_type(*(sai_object_id_t*)lag_id, SAI_OBJECT_TYPE_LAG, &lag_db_id);
    uint32_t jj = 0;
